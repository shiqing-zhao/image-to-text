import { Bt as onUnmounted, It as onMounted, N as Fragment, Nn as effectScope, R as Text, St as inject, W as computed, Xn as shallowRef, _n as watch, dt as getCurrentInstance$1, mt as h, n as vue_runtime_esm_bundler_exports, qn as ref, rt as defineComponent, tt as createVNode, zn as isRef } from "./vue.runtime.esm-bundler-CK7toUjw.js";
...
function datetime(context, ...args) {
	...
	if (!isPlainObject(format$2) || !isString(targetLocale)) return unresolving ? NOT_REOSLVED : key;
...
function translate(context, ...args) {
	...
	if (!resolvedMessage && (!(isString(format$2) || isMessageAST(format$2) || isMessageFunction(format$2)) || !isString(targetLocale))) return unresolving ? NOT_REOSLVED : key;
	if (isString(format$2) && context.messageCompiler == null) {
...
function resolveMessageFormat(context, key, locale, fallbackLocale, fallbackWarn, missingWarn) {
	...
	for (let i = 0; i < locales.length; i++) {
		...
		if (inBrowser) {
			...
			const emitter = context.__v_emitter;
			if (emitter && start && format$2) emitter.emit("message-resolve", {
...
function updateComponentTreeTags(instance, treeNode, i18n) {
	...
	if (instance && instance.__VUE_I18N__) {
		if (instance.__VUE_I18N__ !== global$1) {
			const tag = {
...
function useI18n(options = {}) {
	...
	} else if (scope === "local") warn(getWarnMessage(I18nWarnCodes.DUPLICATE_USE_I18N_CALLING));
	return composer;
}
