import { n as __export } from "./chunk-DNXwb8T0.js";
...
function devtoolsInitApp(app, version$1) {
	emit$1("app:init", app, version$1, {
		Fragment,
		Text,
		Comment,
		Static
...
function watchPostEffect(effect$1, options) {
	return doWatch(effect$1, null, extend({}, options, { flush: "post" }));
}
function watchSyncEffect(effect$1, options) {
	return doWatch(effect$1, null, extend({}, options, { flush: "sync" }));
}
	...
	if (isFunction(ref$1)) callWithErrorHandling(ref$1, owner, 12, [value, refs]);
		...
		if (_isString || _isRef) {
			const doSet = () => {
				...
				} else warn$1("Invalid template ref type:", ref$1, `(${typeof ref$1})`);
			};
			if (value) {
				const job = () => {
					doSet();
  ...
  - expected on client: ${clientText}`);
			...
			if (props) {
				...
				for (const key in props) {
					...
					if (forcePatch && (key.endsWith("value") || key === "indeterminate") || isOn(key) && !isReservedProp(key) || key[0] === "." || isCustomElement && !isReservedProp(key)) patchProp$1(el, key, null, props[key], void 0, parentComponent);
				}
			}
			let vnodeHooks;
...
function createAppAPI(render$1, hydrate$1) {
 ...
 If you want to mount another app on the same host container, you need to unmount the previous app by calling \`app.unmount()\` first.`);
					...
					context.reload = () => {
						...
						cloned.el = null;
						render$1(cloned, rootContainer, namespace);
					};
...
function renderComponentRoot(instance) {
	...
	try {
		if (vnode.shapeFlag & 4) {
			...
			result = normalizeVNode(render$1.call(thisProxy, proxyToUse, renderCache, shallowReadonly(props), setupState, data, ctx));
